...
const languages = [
	...
	{ name: 'julia', language: 'julia', identifiers: ['julia', 'jl'], source: 'source.julia' },
...
const fencedCodeBlockDefinition = (name, identifiers, sourceScope, language, additionalContentName) => {
  ...
  name:
    markup.fenced_code.block.imarkdown
  end:
    (^|\\G)(\\2|\\s{0,3})(\\3)\\s*$
  beginCaptures:
    '3': {name: 'punctuation.definition.imarkdown'}
    '4':
      name: entity.name.type.fenced_code.block.language.imarkdown
    '5':
      name: fenced_code.block.language.attributes.imarkdown
      patterns:
        - {include: '#boolean'}
        - {include: '#dblquoted'}
...
const buildGrammar = () => {
	let text = fs.readFileSync(path.join(__dirname, 'imarkdown.tmLanguage.base.yaml'), "utf8");
	...
	fs.writeFileSync(path.join(__dirname, 'syntaxes', 'imarkdown.tmLanguage'), out);
};
